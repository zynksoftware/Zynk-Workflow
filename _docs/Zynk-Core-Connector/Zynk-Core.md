---
slug: zynk-core
redirect_from: "/article/366-introduction-to-the-zynk-core-library"
title: Zynk Core
---
The Zynk core library contains generic tasks and functions that can be used in your Workflow to facilitate optimal workflow running.

## Tasks
 * [Auto Mapper](auto-mapper)
 * [Check Internet Connection](check-internet-connection)
 * [Container Task](container-task)
 * [Create Backup](create-backup)
 * [Delete Truth Record](delete-truth-record)
 * [Else Task](else-task)
 * [Export Truth Records](export-truth-records)
 * [If Input Contains](if-input-contains)
 * [If Input Equals](if-input-equals)
 * [If Mapping Exists](if-mapping-exists)
 * [If Truth Record Exists](if-truth-record-exists)
 * [Insert Truth Record](insert-truth-record)
 * [Razor Template](razor-template)
 * [Set Error Flag](set-error-flag)
 * [Set Variable](set-variable)
 * [Speak Text](speak-text)
 * [Start Process](start-process)
 * [Start Workflow](start-workflow)
 * [Stop Repeater](stop-repeater) 
 * [Stop Workflow](stop-workflow)
 * [Switch Task](switch-task)
 * [Update Truth Record](update-truth-record)
 * [Upsert Mappings](upsert-mappings)
 * [Upsert Truth Records](upsert-truth-records)
 * [Zynk CSV to Zynk XML](zynk-csv-to-zynk-xml)